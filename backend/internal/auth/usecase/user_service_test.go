...
func TestUserServiceCreateUserRejectsDuplicates(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assertUserEqual(t, got, domain.NewZeroUser())
		})
	}
}
...
func (r *stubUserRepository) Get(_ context.Context, _ domain.UserID) (domain.User, error) {
	if r.getErr != nil {
		return domain.NewZeroUser(), r.getErr
	}
	return r.user, nil
}
...
func (r *stubUserRepository) GetByEmail(_ context.Context, _ string) (domain.User, error) {
	if r.getByEmailErr != nil {
		return domain.NewZeroUser(), r.getByEmailErr
	}
...
func (r *stubUserRepository) Create(_ context.Context, user domain.User) (domain.User, error) {
	if r.createErr != nil {
		return domain.NewZeroUser(), r.createErr
	}
	return user, nil
}

func (r *stubUserRepository) Update(_ context.Context, user domain.User) (domain.User, error) {
	if r.updateErr != nil {
		return domain.NewZeroUser(), r.updateErr
	}
...
func existingUser(t *testing.T) domain.User {
	...
	return domain.NewUser(domain.UserID(id), email, password, domain.NewUserName("John", "Doe"))
}
