...
func TestTaskServiceCreateProjectTaskInheritPriorityProjectNotFound(t *testing.T) {
	...
	if !errors.Is(err, domain.ErrProjectNotFound) {
		t.Fatalf("CreateProjectTask() error = %v, want domain.ErrProjectNotFound", err)
	}
...
func TestTaskServiceGetTaskReturnsAggregate(t *testing.T) {
	repo := &fakeTaskRepository{
		aggregate: domain.TaskAggregate{
			Task: domain.Task{ID: "task-1", UserID: "user-1", Title: "Task"},
			TodoItems: []domain.TodoItem{
				{ID: "todo-1", TaskID: "task-1", Title: "Todo"},
			},
			TaskSchedules: []domain.TaskSchedule{
...
type fakeTaskRepository struct {
	...
	created            domain.Task
	createdInProject   domain.Task
	createInProjectErr error
	taskByUser         domain.Task
	taskByUserErr      error
	updated            domain.Task
	aggregate          domain.TaskAggregate
	aggregateUserID    domain.UserID
	aggregateTaskID    domain.TaskID
	deletedUserID      domain.UserID
	deletedTaskID      domain.TaskID
...
func (r *fakeTaskRepository) CreateInProject(_ context.Context, task domain.Task) (domain.Task, error) {
	r.createdInProject = task
	if r.createInProjectErr != nil {
		return domain.NewZeroTask(), r.createInProjectErr
	}
	return task, nil
}

func (r *fakeTaskRepository) GetByUser(_ context.Context, _ domain.UserID, _ domain.TaskID) (domain.Task, error) {
	if r.taskByUserErr != nil {
		return domain.NewZeroTask(), r.taskByUserErr
	}
	return r.taskByUser, nil
}

func (r *fakeTaskRepository) GetAggregateByUser(_ context.Context, userID domain.UserID, taskID domain.TaskID) (domain.TaskAggregate, error) {
	r.aggregateUserID = userID
	r.aggregateTaskID = taskID
	return r.aggregate, nil
}

func (r *fakeTaskRepository) UpdateByUser(_ context.Context, task domain.Task) (domain.Task, error) {
	r.updated = task
	return task, nil
}

func (r *fakeTaskRepository) DeleteByUser(_ context.Context, userID domain.UserID, taskID domain.TaskID) error {
...
func mustExistingTask(
	...
	task, err := domain.NewExistingTask(id, userID, projectID, title, description, time.Time{}, estimatedMinutes, actualMinutes, progress, priority, status, createdAt, updatedAt)
	if err != nil {
