...
func TestProjectTypeServiceList(t *testing.T) {
	want := []ProjectType{mustProjectTypeForService(t, "work")}
...
func (r *stubProjectTypeRepository) List(ctx context.Context) ([]ProjectType, error) {
	r.listCalled = true
	return r.projectTypes, r.err
}

func mustProjectTypeForService(t *testing.T, value string) ProjectType {
	t.Helper()
